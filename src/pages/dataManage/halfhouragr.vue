...
export default {
    ...
    data() {
        return {
            ...
            searchForm:[
                ...
                {type:'DateTime',label:'开始日期',prop:'daytime.gte',width:'180px',placeholder:'请选择开始日期时间'},
                {type:'DateTime',label:'结束日期',prop:'daytime.lte',width:'180px',placeholder:'请选择结束日期时间'},
            ],
            searchHandle:[ 
    ...
    methods: {
        quertTableDatasCount() {
            ...
            return this.$api.query('orderdata-halfhouragr', {aggregation: 'count', where: buildWhere(this.searchData)}).then(res => {
        ...
        exportExcel(item) {
            ...
            this.exportTableDatas().then(res => {
                ...
                import('@/vendor/Export2Excel').then(excel => {
                    ...
                    const filterVal = ['daytime', 'merchantid', 'planid', 'keywordid', 'totalcount', 'totalpaidcount', 'totalpaidamount', 'hijackcount', 'hijackpaidcount', 'hijackpaidamount']
                    list.forEach((item) => {
